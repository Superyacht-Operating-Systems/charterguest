...
$menuDate = !empty($item['cpm']['Menu_date'])
    ? DateTime::createFromFormat('Y-m-d', $item['cpm']['Menu_date'])
    : false;

if ($menuDate instanceof DateTime) {
    $formattedDate = $menuDate->format('l, j F Y'); // e.g. "Friday, 8 March 2024"
} elseif (!empty($item['cpm']['Menu_date'])) {
    // fallback if the format was different
    $formattedDate = date('l, j F Y', strtotime($item['cpm']['Menu_date']));
} else {
    $formattedDate = ''; // or '-'
}
