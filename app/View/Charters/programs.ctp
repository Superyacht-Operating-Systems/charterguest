...
<section id="sidebar" class="sidebar">
    <nav> 
        ...
        <li class="menu__item" ><a>How To Video</a>
           <ul class="submenu">
                   <li class="menu__item" id="MenuHowToVideo"><a >Preference Sheets</a></li>
                   <li class="menu__item" id="MenuHowToVideoCharterHead"><a>Head Charterer</a></li>
                </ul>
            </li>
            <?php }else{ ?>
...
if(isset($charterGuestData) && !empty($charterGuestData)){
    ...
    //foreach (range(1, 20) as $i) {
        foreach($charterGuestData as $data){
            ...
            $is_news_read = $data['user_news_read'];
            ...
            $date1 = date("Y-m-d", strtotime($data['CharterGuest']['charter_to_date']));
            $date2 = date('Y-m-d');
    ...
    ?>
 <div class="ch-card" <?php if(isset($data['CharterGuest']['cga_display']) && $data['CharterGuest']['cga_display'] == 1){ echo 'style="border: 1px solid #26f5ff;"'; } ?>>
   <div class="ch-card-body">
   <p><?php echo $charterName; ?></p>
    <p><?php echo $charter_from_date; ?> to <?php echo $charter_to_date; ?><?php //echo $is_news_read.'xxxxxxxxxxxxxxxxxxxxxxxxxxxxx';
    if(isset($is_news_read) && $is_news_read == 1){ ?><span class="fa fa-bell guestnew_bell" data-yachtid="<?php echo $yacht_id; ?>" data-cpid="<?php echo $charter_program_id; ?>"></span> <?php } ?></p>
...
if(isset($charterAssocData) && !empty($charterAssocData)){
        foreach($charterAssocData as $data){
            //echo "<pre>"; print_r($data); exit('hhhh');
            ...
            $is_ass_news_read = $data['charterDetails']['user_news_read'];
            $id = $data['charterDetails']['CharterGuest']['id'];
            $charter_program_id = $data['CharterGuestAssociate']['charter_program_id'];
            ...
            $date1 = date("Y-m-d", strtotime($data['charterDetails']['CharterGuest']['charter_to_date']));
            $date2 = date('Y-m-d');
            ...
            $yname = $yfullName[$data['CharterGuestAssociate']['yacht_id']];
    ...
    ?>
<div class="ch-card" <?php if(isset($data['charterDetails']['CharterGuest']['cga_display']) && $data['charterDetails']['CharterGuest']['cga_display'] == 1){ echo 'style="border: 1px solid #26f5ff;"'; } ?>>
   <div class="ch-card-body">
   <p><?php echo $charterName; ?></p>
    <p><?php echo $charter_from_date; ?> to <?php echo $charter_to_date; ?></br><?php //echo $is_ass_news_read.'YYYYYYYYYYYYYYYYYYYY';
    if(isset($is_ass_news_read) && $is_ass_news_read ==1 ){ ?> <span class="fa fa-bell"></span> <?php } ?></p>
    <p><?php echo $embarkation; ?> to <?php echo $debarkation; ?></p>
...
$('.btnNoLink').on('click', function (e) {
	...
	$(document).on("click", ".guestnew_bell" ,function() {
        //alert('jjj');
       
        
        var yachtId = $(this).data('yachtid');//$("#yachtId").val();
        var charprogid = $(this).data('cpid');//$("#charterProgramId").val();
        //alert(yachtId); alert(charprogid);
        //alert(activity_name);
        commenttitlecheck = 0;
         $.ajax({
                 type: "POST",
                 url: BASE_FOLDER+"/"+"charters/getGuestNews",
                 dataType: 'json',
    ...
    $.ajax({
        type: "POST",
        dataType: 'json',
        url: BASE_FOLDER+"/"+"charters/markGuestNewsUnread",
        data: {
            'primaryid': id,
            'read': read,
            'yachtId':yachtId
