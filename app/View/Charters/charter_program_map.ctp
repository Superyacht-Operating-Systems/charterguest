...
$(document).on("click", "#showmenu" ,function() { 
  ...
  if(text == "false"){
    ...
    if(btntext1 == "Open Menu"){
       ...
       // setTimeout(function(){
        $(".routeprocess").css({"top":"234px !important"});
    //},1000);
    }else{
        $("#showmenu").text("Open Menu");
        //setTimeout(function(){
        $(".routeprocess").css({"top":"60px !important"});
   // },1000);
...
if(isset($samelocations[$schedule['CharterProgramSchedule']['lattitude']]) && !empty($samelocations[$schedule['CharterProgramSchedule']['lattitude']])){
    ...
    if($schedule['CharterProgramSchedule']['stationary'] == 1 && $schedule['CharterProgramSchedule']['day_num'] != 1){
        ...
        if($degcount == 1){
         ...
         $stclass = "";
         $rotationAngle = 0;
         // when start and end location are same @apr09 2025
         if(($schedule['CharterProgramSchedule']['day_num'] != 1) && ($schedule['CharterProgramSchedule']['lattitude'] == $debark_lat) && ($schedule['CharterProgramSchedule']['longitude'] == $debark_long)){
            $rotationAngle = 180;
         }
         $degcount = 0;
     }
...
$mar_loc_latitude = $schedule['CharterProgramSchedule']['lattitude'];
    ...
    // }else{
        ...
        if($schedule['CharterProgramSchedule']['stationary'] == 1 && $schedule['CharterProgramSchedule']['day_num'] != 1){
         ...
         // to dispaly the end location number if both start and end locations are same @apr09 2025
         if(($schedule['CharterProgramSchedule']['day_num'] != 1) && ($schedule['CharterProgramSchedule']['lattitude'] == $debark_lat) && ($schedule['CharterProgramSchedule']['longitude'] == $debark_long)){
            //$rotationAngle = 180;
            $textbelowmarkerdeg_new = "text-below-marker-180deg";
         }else{
            $textbelowmarkerdeg_new = "";
         }
    //}
    ?>
 var textbelowmarkerdeg = "<?php echo $textbelowmarkerdeg; ?>";
 var textbelowmarkerdeg_new = "<?php echo $textbelowmarkerdeg_new; ?>"
  ...
  icon: L.divIcon({
    ...
    className: 'text-below-marker '+ textbelowmarkerdeg_new,
    }),
    rotationAngle: 0
    <?php } ?>
...
if(e.target.firstdaytoloc){
    ...
    // new location change @jun17 2025
    var selectedmarkerday_num = e.target.day_num;
    if(selectedmarkerday_num == 1){
        var row_from_lat = e.target._latlng.lat;
        var row_from_long = e.target._latlng.lng;
    }else{
...
if(primaryids.length >= 1){ 
  ...
  //  $(document).ready(function () {
    $(document).on("click", "#CruisingButton", function(e) {
        ...
        var pid = '<?php echo $prgUUID; ?>';
        var yachtdb = '<?php echo $yachtdb; ?>';
        var guesttype = '<?php echo $guesttype; ?>';
        var cp_guesttype = '<?php echo $cp_guesttype; ?>';
        var allow_comments = '<?php echo $allow_comments; ?>';
        $("#hideloader").show();
        //alert(pid);
        $.ajax({
    type: "POST",
    url: basefolder+"/"+"charters/get_cruising_schedule_for_modal",
    dataType: 'json',
    data: { "prgUUID": pid,"yachtdb":yachtdb,"guesttype":guesttype,"cp_guesttype":cp_guesttype,"allow_comments":allow_comments },
    success:function(result) {
        $("#hideloader").hide();
        console.log(result);
        if (result.status == 'success') {
            ...
            $("#cruisinglocationModal").css("display", "block"); 
