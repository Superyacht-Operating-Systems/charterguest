...
class ChartersController extends AppController {
                    ...
                    if (count($charterData) != 0) {
                        ...
                        if (count($charterAssocData) != 0) {
                            ...
                            $charterHeadData = $this->CharterGuest->find('first', array('conditions' => array('charter_program_id' => $charterAssocData['CharterGuestAssociate']['charter_guest_id'])));
                             //$charterHeadData = $this->CharterGuest->find('first', array('conditions' => array('UUID' => $charterAssocData['CharterGuestAssociate']['charter_guest_id'])));
                           ...
                           $this->Session->write("login_username", $loginUser);
                            ...
                            // get the login assosiage details and put in seeaion not based on program @nov17 2025
                            //echo "<pre>"; print_r($GuestListData); print_r($this->Session->read()); //exit;
    ...
    */
    ...
    public function programs() {
        ...
        //$SITE_URL = "https://totalsuperyacht.com:8080/";
        //echo "<pre>";print_r($guestListData); //exit;
        //echo "<pre>"; print_r($CharterGuestConditions);
        ...
        if(isset($charterGuestData) && !empty($charterGuestData)){
            ...
            foreach($charterGuestData as $key => $value){
                ...
                //echo "<pre>";print_r($programFiledata); exit;
                ...
                if(isset($charter_company_id) && !empty($charter_company_id) &&  $charter_company_id != 0){
                    $companyData = $this->Fleetcompany->find('first', array('fields' => array('management_company_name','logo','fleetname','domain_name'), 'conditions' => array('id' => $charter_company_id)));
                    $fleetname = $companyData['Fleetcompany']['fleetname'];
                    $fleet_domain_name = $companyData['Fleetcompany']['domain_name'];
                ...
                $YFileData = $this->CharterGuest->query("SELECT * FROM $ydb_name.charter_program_files CharterProgramFile WHERE charter_program_id = '$puuid' AND is_deleted = 0");
                if(isset($ydb_name) && $ydb_name!='' && !empty($ydb_name)){
                        $cp_news_exist = $this->CharterGuest->query("SELECT * 
                    FROM $ydb_name.charter_program_news CharterProgramNews 
                    WHERE charter_program_uuid = '$puuid' ");
                    //echo "<pre>"; print_r($cp_news_exist); exit;
                    // if news exist then check he read or not
                    if(!empty($cp_news_exist)){
                                        // collect the user read the news or not
                                        $unread_news_count = $this->CharterGuest->query("
                                        SELECT COUNT(*) as unread_count 
                                        FROM $ydb_name.charter_program_news CharterProgramNews 
                                        WHERE charter_program_uuid = '$puuid' 
                                        AND NOT FIND_IN_SET('$usersUUID', user_read_uuid)
                                    ");
                                    /*echo "
                                        SELECT COUNT(*) as unread_count 
                                        FROM $ydb_name.charter_program_news CharterProgramNews 
                                        ...
                                        AND NOT FIND_IN_SET('$usersUUID', user_read_uuid)
                                    "; exit;*/
                                    $unread_count = isset($unread_news_count[0][0]['unread_count']) ? $unread_news_count[0][0]['unread_count'] : 0;
                                
                                    if ($unread_count > 0) {
                                        $user_news_read = 1; // Some news are unread
                                    } else {
                                        $user_news_read = 0; // All news are read
                                    }
                    }else{
                        $user_news_read = 0;
                    }
                ...
                $charterGuestData[$key]['user_news_read'] = $user_news_read;
                //echo "<pre>"; print_r($$user_news_read_status); echo $user_news_read; 
                //echo "<pre>"; print_r($charterGuestData); exit;
                ...
                if(isset($value['CharterGuest']['program_image']) && !empty($value['CharterGuest']['program_image'])){
                    ...
                    //echo $file_folder_path.$domain_name.$fleet_domain_name."<br>";
        ...
        //echo "<pre>";print_r($attachment); echo "<pre>";print_r($programFiles); exit;
         ...
         //echo "<pre>"; print_r($charterAssocConditions);
            //  $charterAssocData = $this->CharterGuestAssociate->find('all', array('conditions' => $charterAssocConditions)); old query
             ...
             if(isset($charterAssocData) && !empty($charterAssocData)){
                ...
                foreach($charterAssocData as $key => $value){
                    ...
                    // collect the user news read or not
                    $cp_uuid = $value['CharterGuestAssociate']['charter_program_id'];
                    $cp_news_exist = $this->CharterProgramFile->query("SELECT * 
                    FROM $ydb_name.charter_program_news CharterProgramNews 
                    WHERE charter_program_uuid = '$cp_uuid' ");
                    //echo "<pre>"; print_r($cp_news_exist); exit;
                    // if news exist then check he read or not
                    if(isset($cp_news_exist) && count($cp_news_exist) > 0){
                                                   // collect the user read the news or not
                                    $unread_news_count = $this->CharterProgramFile->query("
                                    SELECT COUNT(*) as unread_count 
                                    FROM $ydb_name.charter_program_news CharterProgramNews 
                                    WHERE charter_program_uuid = '$cp_uuid' 
                                    AND NOT FIND_IN_SET('$usersUUID', user_read_uuid)
                ...
                //echo "<pre>"; print_r($user_news_read_status); exit;
                $charterAssocData[$key]['charterDetails']['user_news_read'] = $user_news_read;
                   ...
                   // $YachtWeblink = $this->YachtWeblink->find('first', array('conditions' => $YachtWeblinkConditions));
                    ...
                    $this->loadModel('Fleetcompany');
                           ...
                           /* if($value['CharterGuestAssociate']['charter_program_id'] == '677e488b-4a48-4d0f-91bb-4815ac182009'){
                                echo "<pre>";print_r($commentindividualtotal); exit;
                            }*/
                        $charterAssocData[$key]['charterDetails']['msg_count'] = $commentindividualtotal;
                        $commentcounttotal += $commentindividualtotal;
                        $pid = $value['CharterGuestAssociate']['charter_program_id'];
                        $scheduleData = $this->CharterProgramFile->query("SELECT * FROM $ydb_name.charter_program_schedules CharterProgramSchedule WHERE charter_program_id = '$pid' AND is_deleted = 0");
                        // find the charter program data and show @nov14 2025    
                        $cpData = $this->CharterProgramFile->query("SELECT * FROM $ydb_name.charter_programs CharterGuest WHERE UUID = '$pid' AND is_deleted = 0");
                        //echo "<pre>"; print_r($charterAssocData); print_r($cpData); exit;
                        if(isset($cpData) && !empty($cpData)){
                            //$charterAssocData[$key]['charterDetails']['CharterGuest'] = $cpData[0]['CharterGuest'];
                            //$charterAssocData[$key]['charterDetails']['CharterGuest']['charter_from_date'] = $cpData[0]['CharterGuest']['charter_from_date'];
                            //$charterAssocData[$key]['charterDetails']['CharterGuest']['charter_to_date'] = $cpData[0]['CharterGuest']['charter_to_date'];
                            //$charterAssocData[$key]['charterDetails']['CharterGuest']['embarkation'] = $cpData[0]['CharterGuest']['embarkation'];
                            //$charterAssocData[$key]['charterDetails']['CharterGuest']['debarkation'] = $cpData[0]['CharterGuest']['debarkation'];

                            //$charterAssocData[$key]['charterDetails']['CharterGuest']['yacht_id'] = $cpData[0]['CharterGuest']['yacht_id'];
                        }
                        // find the charter program data and show @nov14 2025    
    ...
    // echo "<pre>";print_r($charterGuestData); exit;
         //echo "<pre>";print_r($charterAssocData); print_r($guestListData); exit;
        //echo "<pre>";print_r($yfullName); exit;
    ...
    public function presentations($charterProgramId = null)
    {
        if (!empty($charterProgramId)) {
            ...
            if(!empty($scheduleData)){
                ...
                $location_names = $this->CharterGuest->query("SELECT * FROM $ydb_name.location_contents LocationContent WHERE LocationContent.location = $val_pass AND LocationContent.type = 'Location' AND LocationContent.is_deleted=0");
            ...
            if(isset($scheduleData) && !empty($scheduleData)){
               ...
               foreach($scheduleData as $key => $value){
                //echo $key;
                   // push the to_location to start location
                   if($key == 0){
                    $value['CharterProgramSchedule']['to_location'] = $embarkation_chprg;
                   }
                   ...
                   $LocationContent = $this->CharterGuest->query("SELECT * FROM $ydb_name.location_contents LocationContent WHERE LocationContent.location = $val_pass_locr_temp AND LocationContent.type = 'Location' AND LocationContent.is_deleted=0");
               ...
               //echo "<pre>"; print_r($newschedule); exit('12345');
           ...
           // echo "<pre>"; print_r($scheduleData); exit;
            $i = 0;
                foreach ($scheduleData as $scheduleData) {
                    ...
                    $schduledata[$schduleId.'-'.$i]['GuestMemory'] = $guestmemoryData[0];
                    }
                    $i++;
                }
                //exit;
    ...
    */
    public function crew_list($id='',$charter_program_id='',$fleetcompany_id='',$guesttype='',$cp_guesttype='',$allow_comments='') {
        ...
        $sessionAssoc = $this->Session->read('charter_assoc_info');
         //echo "<pre>";print_r($cp_guesttype);print_r($id); print_r($session); exit;
        if (empty($session)) {
            $this->set('guestType','');
            ...
            // this is opening the preference sheets when user click on crew list button. @nov18 2025
...
//        echo "<pre>";print_r($charterAssocData);exit;
        ...
        if (isset($companyData['Fleetcompany']['logo']) && !empty($companyData['Fleetcompany']['logo'])) {
            ...
            if(isset($fleet_domain_name) && $fleet_domain_name == "totalsuperyacht"){
                // TCY fleet
                $SITE_URL = "https://totalsuperyacht.com:8080/";
                $site_full_path = "/var/www/vhosts/wamp/www/";
            }
            $fleetLogoUrl = $SITE_URL.'/'.$fleetname."/img/logo/thumb/".$companyData['Fleetcompany']['logo'];
        } else{
            $fleetLogoUrl = $SITE_URL.'/'."charterguest/img/logo/thumb/charter_guest_logo.png";
        }
        $this->Session->write("fleetLogoUrl", $fleetLogoUrl);
        ...
        // collecting the crew list from new column and display in crew list @jan 27 2026
        $crewlist = $charterData['CharterGuest']['crewlist'];
        ...
        if (!empty($crewlist)) {
            $sql = "SELECT
                ...
                User.last_name,
                User.image,
                User.crew_order_list,
                User.show_on_cga,
                User.position_id,
                Position.position_name
            FROM " . $ydb_name . ".crew_infos CrewInfo
            LEFT JOIN " . $ydb_name . ".users User ON (CrewInfo.user_id = User.id)
            LEFT JOIN " . $ydb_name . ".positions Position ON (User.position_id = Position.id)
            WHERE CrewInfo.user_id IN (" . $crewlist . ")
            ORDER BY User.crew_order_list ASC, User.first_name ASC";

            $CrewInfo = $this->CharterProgramFile->query($sql);
        }
     ...
     //echo $map_url; echo $guestlink; echo $guesttype; exit;
     ...
     if(isset($YachtWeblinkdata) && !empty($YachtWeblinkdata)){
        ...
        //echo "<pre>";print_r($YachtWeblinkdata); exit;
        //$isValid = 0;
        $isValid = preg_match("@^https?://@", $YachtWeblinkdata[0]['yachts']['cg_yachts_website']);
        if($isValid == 1){
            //$YachtWeblinkdata['YachtWeblink']['weblink'] = 'https://'.$YachtWeblinkdata['YachtWeblink']['weblink'];
            $website = $YachtWeblinkdata[0]['yachts']['cg_yachts_website'];
...
//        echo "<pre>";print_r($charterData);
        ...
        // Fetching the Charter guest associates details
        $this->loadModel('CharterGuestAssociate');
        $charterAssocData = $this->CharterGuestAssociate->find('all', array('conditions' => array('charter_guest_id' => $charter_program_id,'is_email_recipient'=>0,'unselected'=>0)));
        $this->set('charterAssocData', $charterAssocData);
        $unselectedCount = $this->CharterGuestAssociate->find('count', array('conditions' => array('charter_guest_id' => $charter_program_id,'is_email_recipient'=>0,'unselected'=>1)));
        $this->set('unselectedCount', $unselectedCount);
//        echo "<pre>";print_r($charterAssocData);exit;
        ...
        $this->set('prgUUID',$charter_program_id);
        $this->set('yachtdb',$ydb_name);
        $this->set('guesttype','owner');
        $this->set('cp_guesttype','');
        $this->set('allow_comments','');
     //echo "<pre>"; print_r($charterProgData); exit;
     $scheduleData = $this->CharterProgramFile->query("SELECT * FROM $ydb_name.charter_program_schedules CharterProgramSchedule WHERE charter_program_id = '$puuid' AND is_deleted = 0");
     if(isset($scheduleData) && !empty($scheduleData)){
             if(($scheduleData[0]['CharterProgramSchedule']['publish_map'] == 1)){
    ...
    /*
        ...
        * Modified date - 
    */
    public function view_guest($id='',$charter_program_id_params,$charter_company_id_params,$guesttype='',$cp_guesttype='',$allow_comments='') {
        ...
        // Fetching the Head Charterer details
       //echo "<pre>"; print_r($charter_program_id_params); exit;
        $this->Session->delete("Fromownerguestlist");
       ...
       // echo "<pre>";print_r($this->Session->read());exit;
        ...
        if (isset($this->request->data['CharterGuestPersonalDetail']) && !empty($this->request->data['CharterGuestPersonalDetail'])) {
            ...
            if ($this->CharterGuestPersonalDetail->save($data)) {
                ...
                $selectedCharterProgramUUID = $this->Session->read('selectedCharterProgramUUID');
                $this->sendRecordsToYacht($yDBName, $selectedCharterProgramUUID, $charterHeadId, $charterAssocId, $charterGuestAssociateUUID);
                $personalDetailsTab = '';
...
//            echo "<pre>";print_r($data);exit;
            ...
            $yDBName = $session['CharterGuest']['ydb_name'];
            $selectedCharterProgramUUID = $this->Session->read('selectedCharterProgramUUID');
            $this->sendRecordsToYacht($yDBName, $selectedCharterProgramUUID, $charterHeadId, $charterAssocId, $data['guest_lists_UUID']);
            $personalDetailsTab = '';
            $mealPreferenceTab = '';
            $foodPreferenceTab = '';
            $beveragePreferenceTab = '';
...
//            echo "<pre>";print_r($data);exit;
            ...
            if(!empty($frompageleave) && $frompageleave == "submit"){      
                ...
                $updateguest_listsQuery = "UPDATE guest_lists SET is_psheets_done='".$is_psheets_done."',modified='".$modified."',psheet_modified='".$modified."',salutation='".$salutation."' WHERE UUID='$guest_uuid'";
                ...
                if (!empty($guestPersonalData)) {
                    ...
                    //echo "<pre>";echo $guest_uuid; print_r($guestListData); exit;
                    $targetFileName = "";
                    ...
                    if(isset($guestListData)){
                        ...
                        if($guest_file_name != ""){
                                ...
                                if (!empty($guest_file_name)) {
                                    ...
                                    //echo "<pre>";print_r($charter_company_id); exit;
                                    $this->loadModel('Fleetcompany');
                                    $companyData = $this->Fleetcompany->find('first', array('fields' => array('management_company_name','logo','fleetname'), 'conditions' => array('id' => $charter_company_id)));
                    ...
                    if (!empty($fileName)) {
                        ...
                        $this->loadModel('Yacht');
                        $yachtData = $this->Yacht->find('first', array('conditions' => array('id' => $session['CharterGuest']['yacht_id'])));
                        if (isset($yachtData['Yacht']['domain_name']) && $yachtData['Yacht']['domain_name'] == 'charterguest') {
                                $DOC_ROOT = '/var/www/cg-vhost/';
                        }else{
                                $DOC_ROOT = '/var/www/vhosts/wamp/www/';
                        }
                        ...
                        if (!empty($yachtData['Yacht']['fleetname'])) {
                            ...
                            $this->loadModel('Fleetcompany');
                            $fleet_name = $yachtData['Yacht']['fleetname'];
                            $companyData = $this->Fleetcompany->find('first', array('fields' => array('domain_name','logo','fleetname'), 'conditions' => array('fleetname' => "'.$fleet_name.'")));
                            if (isset($companyData['Fleetcompany']['domain_name']) && $companyData['Fleetcompany']['domain_name'] == 'charterguest') {
                                $DOC_ROOT = '/var/www/cg-vhost/';
                            }else{
                                $DOC_ROOT = '/var/www/vhosts/wamp/www/';
                            }
                            $targetFullPath = $DOC_ROOT."/".$yachtData['Yacht']['fleetname']."/app/webroot/".$yachtData['Yacht']['yname']."/app/webroot/".$targetImagePath;
                            $fleettargetFullPath = $DOC_ROOT."/".$yachtData['Yacht']['fleetname']."/app/webroot/img/charter_program_files/".$targetImagePath;
                        } else {
                        ...
                        // copy to the fleet site also
                        $guestDetails = $this->CharterGuest->find('first', array('conditions' => array('charter_program_id' => $selectedCharterProgramUUID)));
                                    //echo "<pre>";print_r($guestDetails);
                                    //exit;
                        $charter_company_id1 = $guestDetails['CharterGuest']['charter_company_id'];
                                    
                                    //echo "<pre>";print_r($charter_company_id); exit;
                        $this->loadModel('Fleetcompany');
                        $companyData = $this->Fleetcompany->find('first', array('fields' => array('management_company_name','logo','fleetname','domain_name'), 'conditions' => array('id' => $charter_company_id1)));


                                    // Fetch the FleetCompany path
                                    
                        if (isset($companyData['Fleetcompany']['domain_name']) && $companyData['Fleetcompany']['domain_name'] == 'charterguest') {
                            $DOC_ROOT1 = '/var/www/cg-vhost/';
                        }else{
                            $DOC_ROOT1 = '/var/www/vhosts/wamp/www/';
                        }
                        $guest_targetFullPath = $DOC_ROOT1."/".$companyData['Fleetcompany']['fleetname']."/app/webroot/img/charter_program_files/";
                        ...
                        if(!file_exists($guest_targetFullPath)){
                            mkdir($guest_targetFullPath,0777,true);
                                    ...
                                    //echo "<pre>";print_r($sourceImagePath);
                                     //echo "<pre>";print_r($guest_targetFullPath);
                        ...
                        if(file_exists($sourceImagePath)){
                            copy($sourceImagePath, $guest_targetFullPath."/".$fileName);
                        }
                    ...
                    if (empty($checkCharterExists)) {
                        ...
                        $updateQuery = "UPDATE $yDBName.passenger_lists SET is_psheets_done='".$is_psheets_done."',salutation='".$salutation."',family_name='".$familyName."',first_name='".$firstName."',date_of_birth='".$dob."',place_of_birth='".$pob."',nationality='".$nationality."',passport_number='".$passportNum."',issued_date='".$issuedDate."',expiration_date='".$expiryDate."',passport_image='".$targetFileName."',file_path='".$targetImagePath."',modified='".$created."',type='".$typetext."',group_id='".$G_group_id."',email='".$guest_email."',token='".$guest_token."',password='".$guest_password."',psheet_modified='".$created."' WHERE UUID='$guest_uuid'"; 
                        $this->CharterGuest->query($updateQuery);
                    }
                    // update password file name to guest list in db_checklistapp
                    if(isset($guestListData)){
                        //echo "<pre>"; print_r($guestListData); print_r($targetFileName); echo $guest_uuid; exit('in');
                        // Updation guest list
                        $updateQuery = "UPDATE guest_lists SET passport_image='".$targetFileName."' WHERE UUID='$guest_uuid'"; //exit;
                        $this->CharterGuest->query($updateQuery);
                    }
                    //exit('out');
                    ...
                    if (empty($checkCharterExists)) {
                        // Insertion
                        $insertQuery = "INSERT INTO $yDBName.passenger_lists (UUID,salutation,family_name,first_name,status,modified,is_psheets_done,type,group_id,file_name,file_path,email,token,password,psheet_modified) VALUES ('".$guest_uuid."','".$salutation."','".$familyName."','".$firstName."',$status,'".$created."','".$is_psheets_done."','".$typetext."','".$G_group_id."','".$guest_targetFileName."','".$guest_targetImagePath."','".$guest_email."','".$Gtoken."','".$Gpassword."','".$created."')";
                        $this->CharterGuest->query($insertQuery);
                    } else {
                        // Updation
                         $updateQuery = "UPDATE $yDBName.passenger_lists SET family_name='".$familyName."',salutation='".$salutation."',first_name='".$firstName."',modified='".$created."',type='".$typetext."',group_id='".$G_group_id."',file_name='".$guest_targetFileName."',file_path='".$guest_targetImagePath."',is_psheets_done='".$is_psheets_done."',email='".$guest_email."',token='".$Gtoken."',password='".$Gpassword."',psheet_modified='".$created."' WHERE UUID='$guest_uuid'"; //exit;
                        $this->CharterGuest->query($updateQuery);
                    }
                }
        ...
        // Fetch psheet_modified from guest_lists for "Preferences last updated" label
        $this->loadModel('GuestList');
        $psheetGuestList = $this->GuestList->find('first', array('fields' => array('psheet_modified'), 'conditions' => array('UUID' => $charterGuestAssociateUUID, 'is_deleted' => 0)));
        $psheetModified = (!empty($psheetGuestList['GuestList']['psheet_modified']) && $psheetGuestList['GuestList']['psheet_modified'] != '0000-00-00 00:00:00') ? date('d M Y', strtotime($psheetGuestList['GuestList']['psheet_modified'])) : '';
        $this->set('psheetModified', $psheetModified);
                ...
                if(isset($data['guestType']) && $data['guestType'] == "guest"){
                    ...
                    if(isset($data['associd']) && !empty($data['associd'])){
                        ...
                        if(($existdata['CharterGuestAssociate']['is_psheets_done'] == 1 && !empty($personalDetails['CharterGuestPersonalDetail']['dob'])) || ((empty($existdata['CharterGuestAssociate']['is_psheets_done']) || $existdata['CharterGuestAssociate']['is_psheets_done'] == 0) && !empty($personalDetails['CharterGuestPersonalDetail']['dob']))){
                            ...
                            $guestListData = $this->GuestList->find('first', array('fields' => array('psheet_modified'), 'conditions' => array('UUID' => $existdata['CharterGuestAssociate']['UUID'], 'is_deleted' => 0)));
                            $result['psheetModified'] = (!empty($guestListData['GuestList']['psheet_modified']) && $guestListData['GuestList']['psheet_modified'] != '0000-00-00 00:00:00') ? date('d M Y', strtotime($guestListData['GuestList']['psheet_modified'])) : 'N/A';
                        }else{
                    ...
                    if(isset($data['associd']) && !empty($data['associd'])){
                        ...
                        if(($CharterGuestexistdata['CharterGuest']['is_psheets_done'] == 1 && !empty($personalDetails['CharterGuestPersonalDetail']['dob'])) || ((empty($CharterGuestexistdata['CharterGuest']['is_psheets_done']) || $CharterGuestexistdata['CharterGuest']['is_psheets_done'] == 0) && !empty($personalDetails['CharterGuestPersonalDetail']['dob']))){ 
                                ...
                                $guestListData = $this->GuestList->find('first', array('fields' => array('psheet_modified'), 'conditions' => array('UUID' => $CharterGuestexistdata['CharterGuest']['users_UUID'], 'is_deleted' => 0)));
                                $result['psheetModified'] = (!empty($guestListData['GuestList']['psheet_modified']) && $guestListData['GuestList']['psheet_modified'] != '0000-00-00 00:00:00') ? date('d M Y', strtotime($guestListData['GuestList']['psheet_modified'])) : 'N/A';
                            }else{
    ...
    */
    public function saveAndSendMail() {
        ...
        if($this->request->is('ajax')){
            ...
            $result = array();
            $sessiondata = $this->Session->read();
            //echo "<pre>";print_r($this->request->data); print_r($sessiondata); exit;
                    ...
                    if (!empty($insertData['existCharterHeadId'])) { //echo "hhh"; exit;
                        ...
                        if (!empty($charterAssocId)) {
                            ...
                            if(empty($guestExistdata)){
                                ...
                                // Initialize $explodeGT as empty array
                                $explodeGT = [];
                                if(isset($exists_guest_type) && !empty($exists_guest_type)){
                                ...
                                //echo "<pre>"; print_r($newsetGTarray); exit('newsetGTarray');
                                ...
                                //echo "<pre>";  print_r($GTimplodeids); exit;
	...
	<p>Please watch this <a href='https://youtu.be/dQFHpxENsPk'>3 min video</a> to learn how to use the charter guest program.</p> 
    ...
    public function sendAlreadySubmittedPreferenceMail($data,$userToken) {
        ...
        <p>Our records show you have previously submitted your preference sheets using charterguest.net and have opted to automatically send them without change to the new charter programs.</p>
        ...
        <p>Watch this <a href='https://youtu.be/5MYJqIhZnHo'>3 min video</a> to learn how to use the charter guest program.</p>    
        </br>
        <p>Kind regards,</p>        
        <p>$captainName</p>       
        <p>$yachtName</p>         
                    ...
                    if ($i == 0) { // Head Charterer
                        ...
                        if(empty($data['charter_assoc_id'][$i])){
                            ...
                            if(isset($guestExistdata) && !empty($guestExistdata)){
                                ...
                                $guestlistData['yacht_id'] = $yachtId;
                                $guestlistData['guest_type'] = $charter_company_id."-".$yachtId."-Guest";
                                $Guestlistuuid = String::uuid();
                                $guestlistData['UUID'] = $Guestlistuuid;
                            }
...
//echo "<pre>"; print_r($guestlistData); exit;
   ...
   /* public function getMealTypeFromTime($time, $menu_type_settings) {
    ...
    public function getMealTypeFromTime($time, $menu_type_settings) {
        //echo $time; echo "<pre>"; print_r($menu_type_settings); exit('dddd');
        if (empty($menu_type_settings) || !isset($menu_type_settings[0]['cga_menu_settings'])) {
            return 'Invalid Configuration'; // Handle cases where menu settings are not properly configured
        }
    //exit('dddddd');
    ...
    private function isTimeWithinPeriod($time, $startTime, $endTime) {
        //echo $time; echo $startTime; echo $endTime; exit;
    ...
    public function charter_program_menus_app($menu_date = null, $menu_time = null ,$yachtdb = null) {
       // echo "<pre>";print_r($_REQUEST);exit('ddddd');
        Configure::write('debug',0);
        $session = $this->Session->read('charter_info');
        $yachtDbName = $yachtdb;
        $menuDate = $menu_date;
        $file = fopen("/var/www/cg-vhost/cga_menu_ipad_inputs.txt","a+");
        fwrite($file,print_r('db:'.$yachtDbName,true));
        fwrite($file,print_r('menuDate:'.$menuDate,true));
        fwrite($file,print_r('time:'.$menu_time. "\n\n",true));
        fclose($file);
        //exit;
        if ($menu_time !== null) {
       ...
       //echo $mealType; exit;
        ...
        //echo $sql1; exit;
               ...
               // echo "<pre>"; print_r($menu_type_settings); exit;
                ...
                // As per the new changes displaying multiple menus @jun23 2025
                // no need to check the mealTYpe so just check isset and allow
        ...
        WHERE (cpm.Menu_date='$menuDate') OR (cpm.Menu_date<='$menuDate' and cpm.every_day_menu=1) and cpm.is_deleted=0 and menu_type=$mealType ORDER BY cpm.modified DESC";
         */
                  $sql = "SELECT cpm.*, cmb.* FROM $yachtDbName.cga_published_menus cpm
        LEFT JOIN $yachtDbName.cga_menu_backgrounds cmb ON cpm.template_id = cmb.UUID
        WHERE cpm.is_deleted=0 and display_on_app=1 ORDER BY cpm.on_app_sequence ASC";
                ...
                if (!empty($yachtDbName)) {
                    ...
                    if (count($charterProgData) != 0) {
                        ...
                        if(!empty($LocationContent)){
                            // new start lat and long @may13 2025
                            //$embark_lat = $LocationContent[0]['LocationContent']['lattitude'];
                            //$embark_long = $LocationContent[0]['LocationContent']['longitude'];
                            $embarkation_chprg_val = $this->CharterGuest->query("
    SELECT * FROM $yachtDbName.charter_program_schedules CharterProgramSchedule 
    WHERE CharterProgramSchedule.title LIKE '%" . addslashes($embarkation_chprg) . "%' 
    AND CharterProgramSchedule.charter_program_id = '" . addslashes($charterProgramId) . "'
");
                            //echo "<pre>"; print_r($embarkation_chprg_val); exit;
                            if (!empty($embarkation_chprg_val)) {
                                $embark_lat =  $embarkation_chprg_val[0]['CharterProgramSchedule']['start_lat'];
                                $embark_long = $embarkation_chprg_val[0]['CharterProgramSchedule']['start_long'];
                            }
                        } else {
                            //$embarkation_chprg_val = $this->CharterProgramSchedule->find('all', array('conditions' => array('title' => $embarkation_chprg)));
                            $embarkation_chprg_val = $this->CharterGuest->query("
    SELECT * FROM $yachtDbName.charter_program_schedules CharterProgramSchedule 
    WHERE CharterProgramSchedule.title LIKE '%" . addslashes($embarkation_chprg) . "%'");
                            if (!empty($embarkation_chprg_val[0])) {
                                // new start lat and long @may13 2025
                                //$embark_lat =  $embarkation_chprg_val[0]['CharterProgramSchedule']['lattitude'];
                                //$embark_long = $embarkation_chprg_val[0]['CharterProgramSchedule']['longitude'];
                                $embark_lat =  $embarkation_chprg_val[0]['CharterProgramSchedule']['start_lat'];
                                $embark_long = $embarkation_chprg_val[0]['CharterProgramSchedule']['start_long'];
                            }
                        }
                        ...
                        if(!empty($scheduleData)){
                            // new location change added bellow loop @jun17 2025
                            foreach($scheduleData as $key => $val){
                                
                        $title_loc = $val['CharterProgramSchedule']['title'];
                        $val_pass_new = '"'.$title_loc.'"';
                        //$myLastElement['CharterProgramSchedule']['title'] = $myLastElement['CharterProgramSchedule']['to_location'];
                        //$location_names = $this->LocationContent->find('first', array('conditions' => array('type' => 'Location', 'is_deleted' => 0, 'location' => $title_loc, 'country !=' => '')));
                        $location_names = $this->CharterGuest->query("SELECT * FROM $yachtDbName.location_contents LocationContent WHERE LocationContent.location = $val_pass_new AND LocationContent.country != ''");
                            //echo "<pre>"; print_r($location_names); exit;
                        // new change start lat & long @may13 2025
                        //$scheduleData[$key]['CharterProgramSchedule']['from_lat'] = $location_names['LocationContent']['lattitude'];
                        //$scheduleData[$key]['CharterProgramSchedule']['from_long'] = $location_names['LocationContent']['longitude'];
    
                        $deb_flag_schedule = $val['CharterProgramSchedule']['debarkation_flag'];
                        $schedule_daynum = $val['CharterProgramSchedule']['day_num'];
                        if($deb_flag_schedule == 1 && $schedule_daynum == 1){
                            $scheduleData[$key]['CharterProgramSchedule']['from_lat'] = $val['CharterProgramSchedule']['start_lat'];
                            $scheduleData[$key]['CharterProgramSchedule']['from_long'] = $val['CharterProgramSchedule']['start_long'];
    
                        }else{
                            $scheduleData[$key]['CharterProgramSchedule']['from_lat'] = $location_names[0]['LocationContent']['lattitude'];
                            $scheduleData[$key]['CharterProgramSchedule']['from_long'] = $location_names[0]['LocationContent']['longitude'];
                           ...
                           $myLastElement['CharterProgramSchedule']['org_title'] = $myLastElement['CharterProgramSchedule']['title'];
                            ...
                            $myLastElement['CharterProgramSchedule']['title'] = $myLastElement['CharterProgramSchedule']['to_location'];
                            $myLastElement['CharterProgramSchedule']['notes'] = $myLastElement['CharterProgramSchedule']['debarkation_desc'];
                            //$myLastElement['CharterProgramSchedule']['day_num'] = $myLastElement['CharterProgramSchedule']['day_num']+1;
                            // //
                            // //echo "<pre>";print_r($scheduleData);
                             $scheduleData[count($scheduleData)] = $myLastElement;
    ...
    public function get_cruising_schedule_for_modal(){
        if($this->request->is('ajax')){
            //echo "<pre>"; print_r($this->request->data); exit;
            $prgUUID = $this->request->data['prgUUID'];
            $yachtdb = $this->request->data['yachtdb'];
            $guesttype = $this->request->data['guesttype'];
            $cp_guesttype = $this->request->data['cp_guesttype'];
        ...
        $yachtDbName = $yachtdb;
        $charterProgramId = $prgUUID;
        $this->set('prgUUID',$prgUUID);
        $this->set('yachtdb',$yachtdb);
        $this->set('guesttype',$guesttype);
        $this->set('cp_guesttype',$cp_guesttype);
        $this->set('allow_comments',$allow_comments);
        if (!empty($yachtDbName)) {
            ...
            if (count($charterProgData) != 0) {
                ...
                // new location changes @jun18 2025
                 if (isset($LocationContent) && !empty($LocationContent)) {
                    // new start lat and long @may13 2025
                   // $embark_lat =  $location_names_emb['LocationContent']['lattitude'];
                    //$embark_long = $location_names_emb['LocationContent']['longitude'];
                      $embarkation_chprg_val = $this->CharterGuest->query("
    SELECT * FROM $yachtDbName.charter_program_schedules CharterProgramSchedule 
    WHERE CharterProgramSchedule.title LIKE '%" . addslashes($embarkation_chprg) . "%' 
    AND CharterProgramSchedule.charter_program_id = '" . addslashes($charterProgramId) . "'
");
                            ...
                            if (!empty($embarkation_chprg_val)) {
                                $embark_lat =  $embarkation_chprg_val[0]['CharterProgramSchedule']['start_lat'];
                                $embark_long = $embarkation_chprg_val[0]['CharterProgramSchedule']['start_long'];
                            }//echo "<pre>"; print_r($embarkation_chprg_val); exit('here');
                ...
                // echo "<pre>";print_r($guestListData); //exit;
                   ...
                   if(isset($scheduleData) && !empty($scheduleData)){
                            ...
                            if($key == 0){
                                ...
                                $loctitle = $value['CharterProgramSchedule']['to_location'];      
                            }
...
//        echo "<pre>";print_r($this->Session->read());exit; 
        ...
        if (!empty($yachtDbName)) {
            ...
            //echo $charterGuestDataToMenu['CharterGuest']['id']; exit;
            $cg_id = $charterGuestDataToMenu['CharterGuest']['id'];
            $this->set('map_url',$map_url);
            ...
            $YachtWeblinkdata = $this->CharterProgramFile->query("SELECT * FROM $yachtDbName.yachts");
           ...
           //echo "<pre>";print_r($YachtWeblinkdata); exit;
            ...
            $this->set('website',$website);
            //$YachtWeblinkdata['Yacht']['cg_yachts_website']
    ...
    WHERE CharterProgramSchedule.title LIKE '%" . addslashes($embarkation_chprg) . "%'");
        ...
        //echo "<pre>"; print_r($scheduleData); exit;
               ...
               // echo "<pre>";print_r($scheduleData); exit;
                ...
                $this->loadModel('Fleetcompany');
                $companyData = $this->Fleetcompany->find('first', array('fields' => array('management_company_name','logo','fleetname','domain_name'), 'conditions' => array('id' => $charterProgData[0]['CharterProgram']['charter_company_id'])));
                if (isset($companyData['Fleetcompany']['logo']) && !empty($companyData['Fleetcompany']['logo'])) {
                    $fleet_domain_name = $companyData['Fleetcompany']['domain_name'];
                    $fleetname = $companyData['Fleetcompany']['fleetname'];
                    if(isset($fleet_domain_name) && $fleet_domain_name == "charterguest"){
               ...
               // $this->set('locationComment', $locationComment);
                ...
                $usersUUID = $this->Session->read("guestListUUID");
                // client asked to add the only related cp charter contract attachments for the cp
                // inside the crusing map page so added the id to the condition @feb25 2025
                // if it is causing any issues for next loops we can  remove it.
                //$CharterGuestConditions = array('users_UUID' => $usersUUID);                
                $CharterGuestConditions = array('users_UUID' => $usersUUID,'id'=>$cg_id);
                $charterGuestData = $this->CharterGuest->find('all', array('conditions' => $CharterGuestConditions, 'order' => 'CharterGuest.charter_from_date desc'));
                ...
                //exit;
                // As per the client we need to load the charter contracts relaed to the cp only so inside the loop
                // added the condition to check $prgUUID @feb25 2025 so over writing with bellow $attachemnt
               ...
               // $this->loadModel('LocationContentFleet');
                if(isset($ipaddb) && !empty($ipaddb)){
                    ...
                    if($last_marker_display_iti_modal == 0){
                        ...
                        $no_of_days_options = "";
                        $modaldisplayDate = "";
                        if(isset($schUUIDs[0]) && !empty($schUUIDs[0])){
                            foreach($schUUIDs as $key => $uuid){
                                ...
                                //echo "<pre>"; print_r($scheduleDataGetNum); exit;
                ...
                $row_debarkation_flag = $scheduleData[0]['CharterProgramSchedule']['debarkation_flag'];
                $charterProgramId = $scheduleData[0]['CharterProgramSchedule']['charter_program_id'];
                ...
                if (count($scheduleData) != 0) {
                    //echo "<pre>"; print_r($scheduleData); exit;
                    if($last_marker_display_iti_modal == 0){
                        if($scheduleData[0]['CharterProgramSchedule']['day_num'] >1){
                    ...
                    // new location changes
                     if(!empty($debarkation_flag) && $debarkation_flag == 1 && $dayNum == 1){
                            // new start lat and long @may13 2025
                            //$embark_lat = $LocationContent[0]['LocationContent']['lattitude'];
                            //$embark_long = $LocationContent[0]['LocationContent']['longitude'];
                            $embarkation_chprg_val = $this->CharterGuest->query("
...
");
                            //echo "<pre>"; print_r($embarkation_chprg_val); exit;
                            if (!empty($embarkation_chprg_val)) {
                                $row_from_lattitude =  $embarkation_chprg_val[0]['CharterProgramSchedule']['start_lat'];
                                $row_from_longitude = $embarkation_chprg_val[0]['CharterProgramSchedule']['start_long'];
                            }
                        } 
                    ...
                    $commentdatatitle = $this->CharterGuest->getCruisingMapComment_new($yachtDbName, $CruisingMapCommentConditons);
                    //}
                    //echo "<pre>"; print_r($commentdatatitle); exit;
                    $guestread = 0;
                      if(isset($commentdatatitle[0]) && !empty($commentdatatitle[0])){
                        foreach($commentdatatitle as $val){
                            if(!empty($val['CruisingMapComment']['guest_read']) && $val['CruisingMapComment']['guest_read'] == "unread"){
                    ...
                    if (count($activityData) != 0) {
                        ...
                        foreach ($activityData as  $activity) {
                            ...
                            if(isset($activityattachment) && !empty($activityattachment)){
                                ...
                                $actname = mysql_real_escape_string($activity['CharterProgramScheduleActivity']['activity_name']);
                                $val_pass = '"'.$title.'"';
                                $LocationContent = $this->CharterGuest->query("SELECT * FROM $yachtDbName.location_contents LocationContent WHERE LocationContent.location = $val_pass AND LocationContent.sub_location = '$actname' AND LocationContent.type != 'Location'");
                                ...
                                if($yname == "yacht"){
                                    $targetFullPath = BASE_URL.'/SOS/app/webroot/betayacht/app/webroot/img/location_contents/'.$activityattachment;
                                    $targetFullGalleryPath = BASE_URL.'/SOS/app/webroot/betayacht/app/webroot/img/location_contents/';
                                    if(BASE_URL == "http://localhost"){
                                       ...
                                       // $targetFullPath = BASE_URL.'/'.$yname."/app/webroot/img/charter_program_files/itinerary_photos/".$activityattachment;
                                        $targetFullPath = $update_BASE_URL.'/'.$yname."/app/webroot/img/location_contents/".$activityattachment;
                                        $targetFullGalleryPath = $update_BASE_URL.'/'.$yname."/app/webroot/img/location_contents/";
                                        if (!empty($fleetSiteName)) { // IF yacht is under any Fleet
                                            $targetFullPath = $update_BASE_URL.'/'.$fleetSiteName."/app/webroot/".$yname."/app/webroot/img/location_contents/".$activityattachment;
                                            $targetFullGalleryPath = $update_BASE_URL.'/'.$fleetSiteName."/app/webroot/".$yname."/app/webroot/img/location_contents/";
                                        }
                                        if(BASE_URL == "http://localhost"){
                                            $targetFullPath = BASE_URL."/superyacht/app/webroot/img/charter_program_files/itinerary_photos/".$activityattachment;
...
<i style="color: #00a8f3;" class="fa fa-solid fa-clock "><span class="icon_label">2h 45m</span></i>
...
<i style="color: #00a8f3;" class="fa fa-solid fa-ship" aria-hidden="true"><span class="icon_label" style="padding: 0px 0px 0px 5px;">45nm</span></i>
	...
	<p><a href='".$cloudURL."'>$cloudURL</a></p>
        ...
        <p>When you complete your preference sheets $headChartererName will be notified by email and your completed preference sheets and personal details will be automatically made available to the Captain of the $yachtName.</p>
	<p>Please watch this <a href='https://youtu.be/5MYJqIhZnHo'>3 min video</a> to learn how to use the charter guest program.</p> 
        <p>We look forward to welcoming you onboard soon.</p>
        </br>
        <p>Kind regards,</p>        
        <p>$captainName</p>       
...
//        curl_setopt($curl, CURLOPT_PROXYUSERPWD, "nagarajan:N@g@r@j@NCeru");
    ...
    */
    ...
    public function getGuestNews() {
         ...
         $login_user_uuid = $this->Session->read('user_login_for_menu');
         ...
         foreach($comments as $key=>$val){
           //$comments[$key]['CharterProgramNew']['guest_read'] = 'unread';
           $user_read_uuid = isset($val['CharterProgramNew']['user_read_uuid']) ? $val['CharterProgramNew']['user_read_uuid'] : '';

            // Convert to an array
            $user_read_array = !empty($user_read_uuid) ? explode(',', $user_read_uuid) : [];

            // Check if the current login user exists in the list
            if (in_array($login_user_uuid, $user_read_array)) {
                $comments[$key]['CharterProgramNew']['guest_read'] = 'read';
            } else {
                $comments[$key]['CharterProgramNew']['guest_read'] = 'unread';
            }
         }
         ...
         $r['view'] = $view->element('charter_program_news', array('news' => $comments));
                ...
                $commentcount = 0;
                  ...
                  if($commentcount > 0){
                    ...
                    foreach($commentdata as $value){
                      if($value['CruisingMapComment']['guest_newlyaddedcomment']==0){  
                            if(trim($value['CruisingMapComment']['guest_read']) == "unread"){  
                                    $facomment="fa";
                                    $colorcode = "red";
                            }
                        }
...
public function markGuestNewsUnread() {
    ...
    $session = $this->Session->read();
    $login_user_uuid = $this->Session->read('user_login_for_menu');
    //echo "<pre>"; print_r($postData); print_r(); exit;
    ...
    if (!empty($check_primary_data)) {
        $current_user_read_uuid = $check_primary_data[0]['CharterProgramNew']['user_read_uuid'];
        $user_read_array = (!empty($current_user_read_uuid)) ? explode(',', $current_user_read_uuid) : [];
        if ($read == "read") {
            // Add login_user_uuid if not already present
            if (!in_array($login_user_uuid, $user_read_array)) {
                $user_read_array[] = $login_user_uuid;
            }
        } else if ($read == "unread") {
            // Remove login_user_uuid if present
            $user_read_array = array_diff($user_read_array, [$login_user_uuid]);
        }
        // Convert array back to comma-separated string
        $updated_user_read_uuid = implode(',', $user_read_array);
        //echo "<pre>"; print_r($updated_user_read_uuid); exit;
         // Prepare update values
         $updateCruisingMapCommentValues = "user_read_uuid='$updated_user_read_uuid', modified=$shipTime";
         $updateConditionsCruisingMapComment = "id = '$primaryid'";
 
         // Update the record
         $this->CharterGuest->updateGuestNews($yachtDbName, $updateConditionsCruisingMapComment, $updateCruisingMapCommentValues);
...
public function markSingleCommentUnread() {
    ...
    $CruisingMapCommentConditons = "id = '$primaryid'";
    $check_primary_data = $this->CharterGuest->getCruisingMapComment($yachtDbName, $CruisingMapCommentConditons);
    $chk_fleet_newlyaddedcomment = $check_primary_data[0]['CruisingMapComment']['fleet_newlyaddedcomment'];
    $chk_crew_newlyaddedcomment = $check_primary_data[0]['CruisingMapComment']['crew_newlyaddedcomment'];
    $chk_guest_newlyaddedcomment = $check_primary_data[0]['CruisingMapComment']['guest_newlyaddedcomment'];
...
public function markCommentUnread() {
    ...
    foreach($postData['ids'] as $key => $val){
        ...
        if($chk_guest_newlyaddedcomment == 0){
            ...
            $updateConditionsCruisingMapComment = "id = '$val'";
            ...
            //echo "<pre>"; print_r($scheduleData); exit;
                            if(isset($scheduleData)){
                                ...
                                foreach($scheduleData as $key => $publishmap){
                                    ...
                                    //to get the to location uuit added condition
                                    if($key == 0){
                                        $loctitle = $publishmap['CharterProgramSchedule']['title'];
                                    }else{
                                        $loctitle = $publishmap['CharterProgramSchedule']['to_location'];      
                                    }
                                    $locationComment[$publishmap['CharterProgramSchedule']['title']]['colorcodetitle'] = $colorcodetitle;
                                     $locationComment[$publishmap['CharterProgramSchedule']['title']]['facomment'] = $facomment;
                                    $locationComment[$publishmap['CharterProgramSchedule']['title']]['programScheduleUUID'] = $programScheduleUUID;
                                    $locationComment[$publishmap['CharterProgramSchedule']['title']]['yacht_id'] = $yacht_id_for_comments;
                                    $locationComment[$publishmap['CharterProgramSchedule']['title']]['act_title'] = $programScheduletitle;
                                    $locationComment[$publishmap['CharterProgramSchedule']['title']]['day_num'] = $daynumber;
                                    $locationComment[$publishmap['CharterProgramSchedule']['title']]['stationary'] = $stationary;
