...
class ChartersController extends AppController {
    ...
    */
    public function index() {
        ...
        // Decrypt encrypted token if present (generated by superyacht share button)
        $guestType   = '';
        $linkSource  = '';
        if (!empty($this->request->query['token'])) {
            ...
            if ($plain !== false) {
                ...
                if (!empty($params['uuid'])) {
                    $this->request->query['uuid'] = $params['uuid'];
                }
                if (!empty($params['guest_type'])) {
                    $guestType = $params['guest_type'];
                }
        ...
        $this->set('linkSource', $linkSource);
    ...
    */
    public function registerGuestUser() {
        ...
        if ($this->request->is('ajax')) {
            ...
            if ($guestType === 'head_charterer') {
                $guestTypeSuffix = ($linkSource === 'fleet') ? 'Head Charterer' : 'Charterer';
            } else {
            ...
            // -------------------------------------------------------
            // 4. YACHT DB: passenger_lists — skip if link came from fleet site
            // -------------------------------------------------------
            if ($linkSource !== 'fleet') {
            ...
            if (in_array($guestType, array('head_charterer', 'Head Charterer', 'owner', 'Owner'))) {
                ...
                $db->query("UPDATE db_checklistapp.charter_guests
                    SET salutation = '{$salutation}',
                        ...
                        email      = '{$usernameSafe}',
                        password   = '{$hashedPwd}'
                    WHERE charter_program_id = '{$charterProgId}'");
            }
     ...
     */
    private function assignCharterToExistingUser($guestData, $charterUuid, $guestType = '', $linkSource = '') {
        try {
            $db = ConnectionManager::getDataSource('default');
            ...
            $guestTypeLabel = ($guestType === 'head_charterer') ? (($linkSource === 'fleet') ? 'Head Charterer' : 'Charterer') : $guestType;
            ...
            // 4. guest_lists — UPDATE existing record to append new charter program values (no duplicates)
            ...
            if (!empty($glRows)) {
                ...
                $existingFleetIds   = array_filter(array_map('trim', explode(',', $gl['fleetcompany_id'])));
                $existingYachtIds   = array_filter(array_map('trim', explode(',', $gl['yacht_id'])));
                $existingGuestTypes = array_filter(array_map('trim', explode(',', $gl['guest_type'])));

                $needsUpdate = false;
                if (!in_array((string)$charterCompanyId, $existingFleetIds)) {
                    $existingFleetIds[] = $charterCompanyId;
                    $needsUpdate = true;
                }
                if (!in_array((string)$yachtId, $existingYachtIds)) {
                    $existingYachtIds[] = $yachtId;
                    $needsUpdate = true;
                }
                if (!in_array($newGuestType, $existingGuestTypes)) {
                    $existingGuestTypes[] = $newGuestType;
                    $needsUpdate = true;
                }

                if ($needsUpdate) {
                    $updFleetcompanyId = implode(',', $existingFleetIds);
                    $updYachtId        = implode(',', $existingYachtIds);
                    $updGuestType      = implode(',', $existingGuestTypes);
                    $glId              = (int)$gl['id'];
