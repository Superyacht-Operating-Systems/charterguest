            ...
            // </div>';
                    ...
                    if (count($activityData) != 0) {
                        foreach ($activityData as $activity) {
                            ...
                            $popupHtml .= '<div class="marksub-div"><div class="sp-60-w"><div><input name="iti_time[]" disabled="true" id="iti_time" style="background-color: #00a8f3;
border-radius: 6px;color: #000;font-size: 15px;border: solid 1px #ccc;width:22% !important;margin: 0px;padding: 8px 5px;font-weight: 600;" value="'.$iti_time.'"><ul class="action-icon"><li><i class="fa fa-comments crew_comment_cruisingmap" style="'.$colorcode.$displaynone.'" data-rel="'.$activity['CharterProgramScheduleActivity']['UUID'].'" data-yachtid="'.$yacht_id.'" data-tempname="'.htmlspecialchars($activity['CharterProgramScheduleActivity']['activity_name']).'" title="Comments & Feedback"><input type="hidden" name=comments[] value="" class="messagecomments" /></i></li></ul></div><input type="text" class="subloc_name" name="activity_name[]" '.$readonly.' value="'.htmlspecialchars($activity['CharterProgramScheduleActivity']['activity_name']).'"><input type="hidden" name="activity_id[]" value="'.$activity['CharterProgramScheduleActivity']['UUID'].'"><textarea class="form-control auto_resize" '.$readonly.' style="background: #fff !important;color: #4d4d4d!important;border: solid 1px rgb(243 243 243 / 70%)!important;" name="messages[]" rows="4" cols="50">'.htmlspecialchars($activity['CharterProgramScheduleActivity']['notes']).'</textarea></div><div class="sp-40-w"><div class="sp-upload-img"><a href="'.$activityattachmentimagehref.'" class="'.$activityfancybox.'"><img src="'.$activityattachmentimage.'" style="object-fit: fill; height: 150px;" alt=""></a></div></div></div>
                             ';
                        }
                    }
